...
function Reader({ article, close }: { article: Article | null; close: () => void }) {
  ...
  function leaf(index: number) {
    ...
    return <><div className="page-running"><span>Dead Women Society</span><span>Nicole</span></div><div className="page-prose">{article.pages[index - 1].map((paragraph, i) => <p key={i}>{paragraph}</p>)}</div><span className="page-number">{index}</span></>;
  }
